...
def fetch_candles(pair, row_count, candle_time, granularity, api: OandaApi, log_message):
    ...
    if df is None or df.shape[0] == 0:
        log_message(f"tech_manager fetch_candles failed to get candles",pair)
        return None
...
def get_trade_decision(candle_time, pair, granularity, api: OandaApi,
    ...
    log_message(f"tech_manager: max_rows: {max_rows} candle_time:{candle_time}, ganularity:{granularity}", pair)
