...
def get_sim_pairs(list_pairs, ic: InstrumentCollection):
    pairs = []
    for p1 in list_pairs:
        for p2 in list_pairs:
            pair = f"{p1}_{p2}"
            if pair in ic.instruments_dict.keys():
                pairs.append(pair)
    return pairs
...
def run_ema_macd(ic: InstrumentCollection):
    ...
    pairs = get_sim_pairs(["USD", "GBP", "JPY", "NZD", "CAD", "AUD"], ic)

    limit = 4 # hardcoded limit for the mutliprocess
    current = 0
